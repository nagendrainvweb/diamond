...
class AddressData {
  ...
  bool validateAddress({Function onError}) {
    if ( address==null || !RegExp(CommonPattern.addressRegex).hasMatch(address)) {
      onError("Please Enter valid Address");
      return false;
    }
    if (area == null || area.isEmpty) {
      onError("Please Enter valid Area");
      return false;
    }
    if (pincode == null || !RegExp(CommonPattern.pincodeRegex).hasMatch(pincode)) {
      onError("Please Enter valid Pincode");
      return false;
    }
    if (state == null || state.isEmpty) {
      onError("Please Enter valid State");
      return false;
    }
    if (city == null || city.isEmpty) {
      onError("Please Enter valid City");
      return false;
    }
    return true;
  }
